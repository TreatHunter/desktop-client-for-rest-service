...
public class UpdateWeaponDialog extends JDialog {
    private boolean createdEntity = false;
    private Weapon weapon;

    UpdateWeaponDialog(Weapon oldWeapon, JFrame owner, List<Brand> allBrands, List<WeaponType> allWeaponTypes, List<Caliber> allCalibers){
        ...
        int weaponTypeIndex = allWeaponTypes.indexOf(allWeaponTypes
                .stream()
                .filter( el -> oldWeapon.getWeaponType().getId() == el.getId())
                .findAny()
                .orElse(null));
        weaponTypedCmb.setSelectedIndex(weaponTypeIndex);
        ...
        int brandIndex = allBrands.indexOf(allBrands
                .stream()
                .filter( el -> oldWeapon.getBrand().getId() == el.getId())
                .findAny()
                .orElse(null));
        ...
        cs.gridwidth = 2;
        panel.add(tfName, cs);
        tfName.setText(oldWeapon.getName());
        ...
        int caliberIndex = allCalibers.indexOf(allCalibers
                .stream()
                .filter( el -> oldWeapon.getBrand().getId() == el.getId())
                .findAny()
                .orElse(null));
        caliberCmb.setSelectedIndex(caliberIndex);
        ...
        tfPrice.setText(String.valueOf(oldWeapon.getPrice()));
        ...
        tfBarcode.setText(String.valueOf(oldWeapon.getBarcode()));
        ...
        tfStatus.setText(oldWeapon.getStatus());
//
